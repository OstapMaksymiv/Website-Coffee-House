...
async function updateCards(event) {
    ...
    const productJson = await fetch('../menu/products.json');
    ...
    if (targetCategory === 'tea') {
        ...
        document.querySelector('.item-5').style.opacity = '0';
        document.querySelector('.item-6').style.display = 'none';
        document.querySelector('.item-6').style.opacity = '0';
        document.querySelector('.item-7').style.display = 'none';
        document.querySelector('.item-7').style.opacity = '0';
        document.querySelector('.item-8').style.display = 'none';
        document.querySelector('.item-8').style.opacity = '0';
...
function switchCardsDisplay(cardNum, action) {
    if (action === 'hide') {
        document.querySelector(`.item-${cardNum}`).style.transition = 'none';
        document.querySelector(`.item-${cardNum}`).style.transform = 'scale(0)';
        document.querySelector(`.item-${cardNum}`).style.opacity = '0';
    } else {
        document.querySelector(`.item-${cardNum}`).style.transition = 'transform 0.5s, opacity 0.5s';
        document.querySelector(`.item-${cardNum}`).style.removeProperty('transform');
        document.querySelector(`.item-${cardNum}`).style.removeProperty('opacity');
    }
}
...
async function openModalMenu(event) {
    ...
    modalImg.style['background-image'] = `url(../images/${productName}.png)`;
